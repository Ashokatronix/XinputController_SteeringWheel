...
void loop() {
  if (compositeHID.isConnected()) {
    ...
    if (break_change > significant_change_threshold) {
      ...
      BreakAdjustedValue = map(BreakVal, 0, 4095, 0, 32737);
    ...
    if (acc_change > significant_change_threshold) {
      ...
      AccAdjustedValue = map(AccVal, 0, 4095, 0, 32737);
    ...
    if (steering_change > significant_change_threshold) {
      // Map analog reading from 0 ~ 4095 to 32737 ~ 0 for use as an axis reading
      SteeringAdjustedValue = map(SteeringVal, 0, 4095, 0, 32737);
      // SteeringAdjustedValue = map(SteeringVal, 0, 4095, -32737, 32766);
